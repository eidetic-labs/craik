...
- :class:`HookBridgeServer` runs inside the craik gateway daemon, listening on a
  ...
  **vendor-agnostic transport decision** (JSON ``{"decision": ...}``). The
  gateway builds that ``decide`` with ``make_operator_decide`` (Task 5.2, in the
  sibling :mod:`craik.runtime.backend.adapters.hook_gating` module), which routes
  the request through craik's operator approval flow.
